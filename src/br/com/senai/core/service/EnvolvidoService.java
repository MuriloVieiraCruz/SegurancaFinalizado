...
public class EnvolvidoService {
	...
	private void validar(Envolvido envolvido) {
		if (envolvido != null) {
			boolean isNomeCompletoInvalida = envolvido.getNomeCompleto() == null
					|| envolvido.getNomeCompleto().isBlank() || envolvido.getNomeCompleto().length() < 2
					|| envolvido.getNomeCompleto().length() > 100;

			if (isNomeCompletoInvalida) {
				throw new IllegalArgumentException("O nome é obrigatório " + "e deve possuir entre 2 e 100 caracteres");
			}

			boolean isDocumentoInvalida = envolvido.getDocumento() == null || envolvido.getDocumento().isBlank()
					|| envolvido.getDocumento().length() > 20;

			if (isDocumentoInvalida) {
				throw new IllegalArgumentException("O documento é " + "obrigatória e deve possuir até 20 caracteres");
			}
	...
	public void excluirPor(int idDoEnvolvido) {
		if (idDoEnvolvido > 0) {
	...
	public List<Envolvido> listarTodas() {
		return daoEnvolvido.listarTodas();
	}
}
